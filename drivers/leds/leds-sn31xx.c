...
{                               
 ...
 #ifdef CONFIG_TCT_8X16_TAPTI /*[BUGFIX]-Add by TCTNB.XQJ,PR-958858 ,2015/04/07,add new blink time,about 0.5+2s*/
	else  if (bblink==3) {
              pr_err("blink on mode 3 \n");
              sn31xx_write_byte(led->client,0x2f, 0x00); //reset
              sn31xx_write_byte(led->client,0x02, 0x20); //led mode
              sn31xx_write_byte(led->client,0x00, 0x20);
		sn31xx_write_byte(led->client,0x03,0x08); //i max 10ma
              sn31xx_write_byte(led->client,0x04, 0x01);
              sn31xx_write_byte(led->client,0x07, 0xff);
              sn31xx_write_byte(led->client,0x0a, 0x00); //T0 1S
              sn31xx_write_byte(led->client,0x10, 0x06);//T1,Tt2,T1 0.13,T2,0.26,
              sn31xx_write_byte(led->client,0x16, 0x0a);//T3,T4 ,T3,0.13,t4,2s
              sn31xx_write_byte(led->client,0x1C, 0x00);//UPDATE
       }
#endif
}
...
#ifndef CONFIG_TCT_8X16_IDOL3 /*[BUGFIX]-Mod by TCTNB.XQJ,PR-817507 2014/10/23,not need in idol3*/
