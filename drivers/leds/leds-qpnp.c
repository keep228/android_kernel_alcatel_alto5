...
/* [PLATFORM]-Mod-BEGIN by TCTNB.CY, PR-893569, 2015/1/10, control backlight brightness*/
#define BRIGHTNESS_MAX_SIZE 11 //tianhongwei 2014/11/03 PR.816765
static int qpnp_lcdbl_gpio_set(struct qpnp_led_data *led)
{
	...
	unsigned long flags;
//TCTNB.CY,remove for  PR-945747, 2015/3/31
#endif
	if(0 == led->cdev.brightness){
		gpio_direction_output(led->lcdbl_gpio_cfg->gpio_num,0);
		level = 0;
	}else{
		bl_level = (int)(led->cdev.brightness * BRIGHTNESS_MAX_SIZE / led->cdev.max_brightness) + 1;		

		if (bl_level > BRIGHTNESS_MAX_SIZE + 1) {
			bl_level = BRIGHTNESS_MAX_SIZE + 1;
		}

		if (0 == level ){
/* [PLATFORM]-Mod-BEGIN by TCTNB.CY, PR-945747, 2015/3/31, add delay for Screen flickers during calling*/							
			#ifdef CONFIG_TCT_8X16_ALTO4
			mdelay(50);
			local_irq_save(flags);
			#endif
/* [PLATFORM]-Mod-END by TCTNB.CY, 2015/3/31*/
			gpio_direction_output(led->lcdbl_gpio_cfg->gpio_num,0);
			udelay(10);
			gpio_direction_output(led->lcdbl_gpio_cfg->gpio_num,1);
			udelay(30);
...
/* [PLATFORM]-Mod-END by TCTNB.CY, 2015/1/10*/
