...
/* INFO: SGM3141 chip operations
				state        state
Flash_Torch pin       1            0
Flash_EN     pin       1            PWM
Chip mode           Flash         Torch
*/
...
static int led_pwm_control(struct flash_driver_info *flash_led, enum led_brightness value)
{
	int brightness = value;

	pub_msm_tlmm_v4_gp_fn_gpio_32(PWM_PIN,0,1);
	gpio_direction_output(flash_led->flash_en, 0);

	if (brightness > LED_HALF) {
		pub_msm_tlmm_v4_gp_fn_gpio_32(PWM_PIN,0,1);
		gpio_direction_output(flash_led->flash_en, 1);
		flash_led->clk_prepared = 0;

	} else if(brightness > LED_OFF){
		pub_msm_tlmm_v4_gp_fn_gpio_32(PWM_PIN,2,1);
		if (brightness > 85) {
...
static void led_torch_pwm_work(struct work_struct *work)
{
	...
	flash_led = container_of(work, struct flash_driver_info, pwm_work);

	if (flash_led) {
		mutex_lock(&flash_led->lock);
		gpio_direction_output(flash_led->flash_torch, 0);
		led_pwm_control(flash_led, flash_led->cdev_torch.brightness);
...
static void led_flash_work(struct work_struct *work)
{
	struct flash_driver_info *flash_led =NULL;
	int value = 0;
	flash_led = container_of(work, struct flash_driver_info, flash_work);

	if (flash_led) {
		value = flash_led->cdev_flash.brightness;
...
static void led_gpio_brightness_set(struct led_classdev *led_cdev, enum led_brightness value)
{
	...
	if (0 == strncmp(led_cdev->name, "led-flash", strlen("led-flash"))) {
		...
		if (flash_led) {
			schedule_work(&flash_led->flash_work);
		} else
...
static int led_gpio_flash_probe(struct platform_device *pdev)
{
	...
	if (flash_led->flash_torch < 0) {
		...
		rc = gpio_request(flash_led->flash_torch, "FLASH_TORCH");
		if (rc) {
			printk("%s: Failed to request gpio %d, rc = %d\n",
				__func__, flash_led->flash_torch, rc);
			goto error;
	...
	INIT_WORK(&flash_led->flash_work, led_flash_work);
...
MODULE_DESCRIPTION("SGM3141 driver");
