...
#include "s5k5e2_alto5_v4l2_tct_OTP.h"
...
/*[PLATFORM]-Add-BEGIN by TCTNBYQJ, FR-844467, 2014/12/11 */
	if(!strcmp(s_ctrl->sensordata->sensor_name, "s5k4h5yc_alto5"))
	{
		bu6424_close();
	}
	if (!strcmp(s_ctrl->sensordata->sensor_name, "ov8858_alto5"))
	{
		dw9714_close();
	}
...
int msm_sensor_match_id(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if (!strcmp(s_ctrl->sensordata->sensor_name, "s5k5e2_alto4"))
	{
		...
		if(actutor_rc == 0){
			actuator_exist = 1;
			printk("s5k5e2_alto4 have actutor,it is AF module \n");
		}else
		{
			actuator_exist = 0;
			printk("s5k5e2_alto4 have no actutor,it is FF module \n");
		}
		//S5K5E2_ALTO45_CMCC_TCT_OTP_get_data(sensor_i2c_client);
	}
...
extern int32_t ov5670_otp_config(struct msm_sensor_ctrl_t * s_ctrl);
#endif
...
static int TCT_update_OTP(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if (!strcmp(s_ctrl->sensordata->sensor_name, "ov5670_alto4evdo")) {
	...
	if (!strcmp(s_ctrl->sensordata->sensor_name, "ov5670_truly_ff") ||
...
#ifndef CONFIG_TCT_8X16_POP10
	else if ( (!strcmp(s_ctrl->sensordata->sensor_name, "ov8858_alto5")) ||(!strcmp(s_ctrl->sensordata->sensor_name, "ov8858_idol3")) ){
		...
		printk("OTP: s5k5e2_alto45\n");
		S5K5E2_ALTO45_TCT_OTP_calibration(s_ctrl->sensor_i2c_client);
	}
	else if(!strcmp(s_ctrl->sensordata->sensor_name, "s5k5e2_alto5")){
		printk("OTP: s5k5e2_alto5\n");
		S5K5E2_ALTO5_TCT_OTP_calibration(s_ctrl->sensor_i2c_client);
	}
	...
	else if(!strcmp(s_ctrl->sensordata->sensor_name, "s5k5e2_premium")){
		printk("OTP: s5k5e2_premium,it is front camera\n");
		S5K5E2_SUNNY_TCT_OTP_calibration(s_ctrl->sensor_i2c_client);
	}
#endif
