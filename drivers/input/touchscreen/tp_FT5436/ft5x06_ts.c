...
#ifdef CONFIG_FT6336_TOUCHPANEL_USB_DETECT
static struct workqueue_struct *ft_usbdet_wq;
#endif
...
/*[BUGFIX]-Add by TCTNB.ZXZ, PR951497,2015/03/17, add reset after resume*/
	if (gpio_is_valid(data->pdata->reset_gpio)) {
		gpio_set_value_cansleep(data->pdata->reset_gpio, 0);
		msleep(data->pdata->hard_rst_dly);
		gpio_set_value_cansleep(data->pdata->reset_gpio, 1);
		...
		queue_work(ft_usbdet_wq, &data->work);
	#endif
		return 0;
	}
#endif
 ...
 void ft_change_scanning_frq_switch(struct work_struct *work)
 {
 ...
 //      printk(KERN_ERR"changer_in_flag  = %d \n",changer_in_flag);
		...
		pr_err("========= switch========data->changer_in=%d====\n",data->changer_in);
...
input_set_capability(input_dev, EV_KEY, TCT_KEY_MENU);
 ...
 #ifdef CONFIG_FT6336_TOUCHPANEL_USB_DETECT
     INIT_WORK(&data->work, ft_change_scanning_frq_switch);
 #endif
...
static int __init ft5x06_ts_init(void)
{
 ...
 #ifdef CONFIG_FT6336_TOUCHPANEL_USB_DETECT
     ft_usbdet_wq = create_singlethread_workqueue("ft_usbdet_wq");
     if (!ft_usbdet_wq)
     {
         printk("Creat ft_usbdet_wq workqueue failed. \n");
         return -ENOMEM;
     }
 #endif
...
static void __exit ft5x06_ts_exit(void)
{
         ...
         if (ft_usbdet_wq)
         {
                 destroy_workqueue(ft_usbdet_wq);
         }
 #endif
