				...
				struct device_attribute *attr, char *buf)
{
	...
	if (err < 0)
		dev_err(&client->dev, "vendor read error");

	reg_addr = 0xA6;
	err = ft6x06_i2c_read(client, &reg_addr, 1, &data->fw_ver[0], 1);
	if (err < 0)
		dev_err(&client->dev, "fw_version read error");
 ...
 int ft6x06_ts_resume(struct device *dev)
{
	...
	if ( data->gesture_id > 0) {
		...
		/*[BUGFIX]-Add by TCTNB.ZXZ, PR948464,2015/03/16, add reset after resume*/
	if (gpio_is_valid(data->pdata->reset_gpio)) {
		gpio_set_value_cansleep(data->pdata->reset_gpio, 0);
		msleep(data->pdata->hard_rst_dly);
		gpio_set_value_cansleep(data->pdata->reset_gpio, 1);
...
/*[BUGFIX]-Add by TCTNB.ZXZ,PR948464,2015/03/16, IRQF_TRIGGER_FALLING add for TP doule click can wake up when phone in sleep*/
