...
//zxz modify for do firmware update directly not use queue, queue may cause update crash
#if 0
...
#else
void _DrvFwCtrlUpdateFirmwareBySwIdDoWork(void)
#endif
{
    s32 nRetVal = 0;
...
void DrvFwCtrlCheckFirmwareUpdateBySwId(void)
{
  ...
  //zxz modify for do firmware update directly not use queue, queue may cause update crash
    ...
    if (nCrcMainA == nCrcMainB && nCrcInfoA == nCrcInfoB) // Case 1. Main Block:OK, Info Block:OK
    {
        ...
        if (eSwId == MSG26XXM_SW_ID_XXXX||eSwId == MSG26XXM_SW_ID_0001)
        {
