...
#include "ft6x06_ts.h"
/*[PLATFORM]-Mod-begin by TCTNB.ZXZ, PR963130, 2015/04/21, add for TP Psensor,can't light up LCD when calling*/
...
#ifdef CONFIG_FT6336_TOUCHPANEL_USB_DETECT
       struct work_struct      work;
       u8 changer_in;
#endif
/*[PLATFORM]-Mod-begin by TCTNB.ZXZ, PR963130, 2015/04/21, add for TP Psensor,can't light up LCD when calling*/
    struct wake_lock ft_nosuspend_wl;
/*[PLATFORM]-Mod-end*/
};


#ifdef CONFIG_FT6336_TOUCHPANEL_USB_DETECT
static struct workqueue_struct *ft6x06_wq;
#endif
				...
				struct device_attribute *attr, char *buf)
{
	...
	reg_addr = 0xA8;
	err = ft6x06_i2c_read(client, &reg_addr, 1, &reg_data, 1);
	if (err < 0)
				...
				struct device_attribute *attr, char *buf)
{
	...
	reg_addr = 0x96;
	err = ft6x06_i2c_read(client, &reg_addr, 1, &reg_data0, 1);
	if (err < 0)
		dev_err(&client->dev, "reg_data0");

	reg_addr = 0xB0;
	err = ft6x06_i2c_read(client, &reg_addr, 1, &reg_data1, 1);
	if (err < 0)
...
void tp_gestures_register ( struct ft6x06_ts_data *data)
{
	int rc = 0;
...
#if defined(FOCALTECH_TP_GESTURE)
static int ft_tp_interrupt(struct ft6x06_ts_data *data)
{
	int rc = 0;
...
//[PLATFORM]-Mod-begin by TCTNB.ZXZ, FR-884189, 2014/12/29, add for TP ESD recovery
#ifdef CONFIG_FT_TOUCHPANEL_ESD_RESET
  if(rc==-ETIMEDOUT)
	{
		...
		pr_err("FT==========ESD===had reset=========\n");
		msleep(data->pdata->soft_rst_dly);

		rc = ft6x06_i2c_read(data->client, &reg, 1,buf, data->tch_data_len);
...
//[PLATFORM]-Mod-End by TCTNB.ZXZ
	...
	if (rc < 0) {
		dev_err(&data->client->dev, "%s: read data fail====rc=%d=\n", __func__,rc);
		return IRQ_HANDLED;
	}
	...
	{
/*[PLATFORM]-Mod-begin by TCTNB.ZXZ, PR963130, 2015/04/21, add for TP Psensor,can't light up LCD when calling*/
      wake_lock_timeout(&data->ft_nosuspend_wl, 1*HZ);
/*[PLATFORM]-Mod-end*/
	...
	else
	{
		...
		//pr_err("zxz=========== =====tpd_proximity_detect=%d====\n",tpd_proximity_detect);

		input_report_abs(vps->virtualdevice, ABS_DISTANCE, tpd_proximity_detect);
	...
	for (i = 0; i < data->pdata->num_max_touches; i++) {
		...
		if (status == FT_TOUCH_DOWN || status == FT_TOUCH_CONTACT) 
		{
			if(y >= 900)
			{
			/*modify(add) by Bin.Su.nb for Alto 5 GL 2nd TP keyboard can not test with CR875134 begin . 201401223*/
				...
				#endif
				/*modify(add) by Bin.Su.nb for Alto 5 GL 2nd TP keyboard can not test with CR875134 end . 201401223*/
			}
			else{
...
#if defined(FOCALTECH_TP_GESTURE)
static int ft_tp_suspend(struct ft6x06_ts_data *data)
{
	char txbuf[2];
	int err = 0;
	...
	if ( data->gesture_id > 0)
	{
		...
		for (i = 0; i < 10; i ++){
			...
			ft6x06_i2c_read(data->client, &reg, 1, &reg_value, 1);
			//if (0x55 == reg_value)
			if (0x01 == reg_value)
				break;
		}
...
static int ft_tp_resume(struct ft6x06_ts_data *data)
{
	char txbuf[2];
	if ( data->gesture_id > 0)
	{
		...
		msleep(100);
	#if defined(CONFIG_FT6336_TOUCHPANEL_USB_DETECT)
		queue_work(ft6x06_wq, &data->work);
	#endif
		return 0 ;
	}
	#if defined(CONFIG_FT6336_TOUCHPANEL_USB_DETECT)
 ...
 int ft6x06_ts_resume(struct device *dev)
{
	...
	if(tpd_proximity_flag)
	{
		...
		/*[BUGFIX]-Add by TCTNB.ZXZ, PR888110,2015/02/05, add for virtual Psensor can't wake up lcd when make call*/
			if (device_may_wakeup(&data->client->dev))
			disable_irq_wake(data->client->irq);
		/*[BUGFIX]End-Add by TCTNB.ZXZ*/
			return 0;
	...
	if ( data->gesture_id > 0) {
		...
		/*[BUGFIX]-Add by TCTNB.ZXZ, PR951739,2015/03/16, add reset after resume*/
	if (gpio_is_valid(data->pdata->reset_gpio)) {
		gpio_set_value_cansleep(data->pdata->reset_gpio, 0);
		msleep(data->pdata->hard_rst_dly);
		gpio_set_value_cansleep(data->pdata->reset_gpio, 1);
...
rel_fw:
...
#ifdef CONFIG_FT6336_TOUCHPANEL_USB_DETECT
  void ft6x06_enable_change_scanning_frq(void)
 {
	...
	if(probe_ok==false)
		return;
         ...
         data = i2c_get_clientdata(ft_g_client);
 ...
 void ft6x06_disable_change_scanning_frq(void)
 {
         struct ft6x06_ts_data *data;
         if(!ft_g_client)
                 return ;
 ...
 void ft6x06_change_scanning_frq_switch(struct work_struct *work)
 {
         u8 changer_in_flag = 0;
         struct ft6x06_ts_data *data;
 //      printk(KERN_ERR"[TPD]%s, %d\n", __FUNCTION__, __LINE__);
         data = container_of(work, struct ft6x06_ts_data, work);
         if (data->suspended)
         {
 //              printk(KERN_ERR"changer_in  = %d \n",data->changer_in);
       ...
       if(ft_g_client==NULL)
           return ;
         ft5x0x_read_reg(ft_g_client, 0x8b, &changer_in_flag);
 //      printk(KERN_ERR"changer_in_flag  = %d \n",changer_in_flag);
         if(changer_in_flag != data->changer_in)
         {
                 printk(KERN_ERR"[ft6x06]%s: Write %d to 0x8b\n", __FUNCTION__, data->changer_in);
                 ft5x0x_write_reg(ft_g_client, 0x8b, data->changer_in);
         }
 }
 #endif
...
int board_id_detect(void)
{
	int rc = 0;

	rc = gpio_request(GPIO_BOARD_ID4, "board_id0");
	if (rc) {
		pr_err("==board_id_detect=======request gpio17 failed, rc=%d\n", rc);
		goto err_gpio17;
	}
	...
	gpio_free(GPIO_BOARD_ID4);
		pr_err("==board_id_detect=========gpio_get_value17==%d========\n", rc);
return rc;
			   ...
			   const struct i2c_device_id *id)
{
 ...
 #ifdef CONFIG_FT6336_TOUCHPANEL_USB_DETECT
     INIT_WORK(&data->work, ft6x06_change_scanning_frq_switch);
 #endif

/*[PLATFORM]-Mod-begin by TCTNB.ZXZ, PR963130, 2015/04/21, add for TP Psensor,can't light up LCD when calling*/
    wake_lock_init(&data->ft_nosuspend_wl,WAKE_LOCK_SUSPEND, "ft_int_wakelock");
/*[PLATFORM]-Mod-end*/
...
//decide need upgrade_firmware
    ...
    //add by junfeng.zhou for update_project end .
		/*[BUGFIX]-Add by TCTNB.ZXZ, FR824239,2015/03/02, IRQF_TRIGGER_FALLING add for TP doule click can wake up when phone in sleep*/
...
/*[PLATFORM]-Mod-begin by TCTNB.ZXZ, PR963130, 2015/04/21, add for TP Psensor,can't light up LCD when calling*/
    wake_lock_destroy(&data->ft_nosuspend_wl);
/*[PLATFORM]-Mod-end*/
...
static int __init ft6x06_ts_init(void)
{
 ...
 #ifdef CONFIG_FT6336_TOUCHPANEL_USB_DETECT
     ft6x06_wq = create_singlethread_workqueue("ft6x06_wq");
     if (!ft6x06_wq)
     {
         printk("Creat ft6x06 workqueue failed. \n");
         return -ENOMEM;
     }
 #endif
