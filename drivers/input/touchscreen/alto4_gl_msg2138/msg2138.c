...
//[PLATFORM]-Add-BEGIN by TCTNB.ZXZ, FR-939401 2015/03/06,add for TP Psensor
...
#include <linux/sensors.h>
 u8 tpd_proximity_flag2 			= 0;
 u8 tpd_proximity_flag_one2 		= 0;
 u8 tpd_proximity_detect2 		= 1;//0-->close ; 1--> far away
...
extern struct virtualpsensor2 *vps2;
extern struct sensors_classdev virtual_sensors_proximity_cdev2;
extern  int virtual_psensor_input_register2(struct i2c_client *pClient);
extern int vps_set_enable2(struct sensors_classdev *sensors_cdev,unsigned int enable);
extern void set_psensor_register(int status);
extern u8 get_psensor_register(void);
...
struct msg2138_ts_data {
	    ...
	    bool in_suspend;
/*[BUGFIX]-Begin Add by TCTNB.ZXZ, PR983680,2015/04/21, add for virtual Psensor can't wake up lcd when make call*/
    struct wake_lock msg_nosuspend_wl;
/*[BUGFIX]-End Add by TCTNB.ZXZ*/
#if defined(CONFIG_FB)
...
//[PLATFORM]-TCTNB.ZXZ, FR-939401 2015/03/06,add for TP Psensor ,for extern
...
//[PLATFORM]-Add-BEGIN by TCTNB.ZXZ, FR-803440 2014/11/27,make double click in the same path
static DEVICE_ATTR(tp_gesture_id, 0644, hw_tp_double_click_show, hw_tp_double_click_store);
//[PLATFORM]-Add-END by TCTNB.ZXZ
//modify(add) by junfeng.zhou.sz for [TP driver development(1st source) for alto5] with PR721418 end .
...
/*[PLATFORM]-Add-BEGIN by TCTNB.ZXZ, PR929466 2015/02/13 ,force FW update if it's damaged */
#if defined(CONFIG_TCT_8X16_ALTO5)
#define TP_IC_MAJOR_VERSION  5
...
/*[PLATFORM]-Add-BEGIN by TCTNB.ZXZ, PR946890 2015/03/11,phone may have I2C error can cause phone crash when update firmware */
#if 0
...
{
/*modify (add) wangxingchen for PR 884993 02/10/2015*/
//    #include "include/firmware/ALTO4GL_EACH/ALTO4GL_2138A_Each_V5.02_140826.h"
//    #include "include/firmware/ALTO4GL_EACH/ALTO4GL_2138A_Each_V5.05_141202.h"
//	#include "include/firmware/ALTO4GL_EACH/ALTO4GL_2138A_Each_V5.07_150210.h"
//	#include "include/firmware/ALTO4GL_EACH/ALTO4GL_2138A_Each_V5.09_150305.h"
//	#include "include/firmware/ALTO4GL_EACH/ALTO4GL_2138A_Each_V5.10_150318.h"
//	#include "include/firmware/ALTO4GL_EACH/ALTO4GL_2138A_Each_V5.12_150327.h"
//	#include "include/firmware/ALTO4GL_EACH/ALTO4GL_2138A_Each_V5.13_150407.h"
	#include "include/firmware/ALTO4GL_EACH/ALTO4GL_2138A_Each_V5.14_150413.h"
};
...
unsigned char MSG_2133A_BIN[94*1024] =
{
      #include "include/firmware/ALTO5GL_EACH/ALTO5GL_2138A_Each_V5.10_150126.h"
};
#else
//use ALTO4NA
unsigned char MSG_2133A_BIN[94*1024] =
...
//modify(add) by junfeng.zhou.sz for [TP driver development(1st source) for alto5] with PR721418 end . 
//[PLATFORM]-modify by TCTNB.ZXZ, FR-939401 2015/03/06,add for TP Psensor for extern
...
#if defined(CONFIG_TCT_8X16_ALTO4)
/*modify (add) wangxingchen for PR 884993 02/10/2015*/
	unsigned short version_list[] =
	{0x00,0x01,0x02,0x03,0x04,0x05,0x06,0x07,0x09,0x0a,0x0b,0x0c,0x0d};	//Keep the old version (minor) list that allows to be updated.
#else
...
if(TP_IC_MAJOR_VERSION!= major)
{
    ...
    pr_err("=======version_cat======firmware error need force update it !=======\n");
...
//[PLATFORM]-Add-BEGIN by TCTNB.ZXZ, FR-939401 2015/03/06,add for TP Psensor
	...
	if(tpd_proximity_flag2)
	{

	#if 0
		reg_value=get_psensor_register();
	
		if(reg_value!=1)
		{
			vps_set_enable2(&virtual_sensors_proximity_cdev2,1);
		}
	#endif
/*[BUGFIX]-Begin Add by TCTNB.ZXZ, PR983680,2015/04/21, add for virtual Psensor can't wake up lcd when make call*/
      wake_lock_timeout(&msg2138_data->msg_nosuspend_wl, 1*HZ);
/*[BUGFIX]-End Add by TCTNB.ZXZ*/
    if(( val[0] == 0x52) && ( val[1] == 0xFF ) && ( val[2] == 0xFF ) && ( val[3] == 0xFF ) && ( val[4] == 0xFF ) && ( val[6] == 0xFF ) && ( Checksum == val[7] ))
	{
		is_face_detect = val[5];
		//close=0x80,far=0x40
		if(is_face_detect==0x80)
		{
			tpd_proximity_detect2 = 0;
		}
		else if(is_face_detect==0x40)
		{
			tpd_proximity_detect2 = 1;

		}
	}

		input_report_abs(vps2->virtualdevice, ABS_DISTANCE, tpd_proximity_detect2);
...
#endif
//modify(add) by junfeng.zhou.sz for [TP driver development(1st source) for alto5] with PR721418 end .
/*[PLATFORM]-modify by TCTNB.ZXZ, PR929466 2015/02/13 ,FAE suggest the reset time had better change like "ito_test_reset" */
...
//[PLATFORM]-Add-BEGIN by TCTNB.ZXZ, FR-939401 2015/03/06,add for TP Psensor
	...
	if(tpd_proximity_flag2)
	{
		tpd_proximity_flag_one2= 1;
		/*[BUGFIX]Begin-Add by TCTNB.ZXZ,2015/02/05, add for virtual Psensor can't wake up lcd when make call*/
		if (device_may_wakeup(&msg2138_data->client->dev))
		err=enable_irq_wake(msg21xx_irq);
		/*[BUGFIX]End-Add by TCTNB.ZXZ*/
		return 0;
	}
#endif
...
//[PLATFORM]-Add-BEGIN by TCTNB.ZXZ, PR-967705 2015/04/07,add for TP suspend flag ,
//there are two resume when press power key when call (enable proximity function resume and power key resume )the second resume will reset the proximity register
	if (msg2138_data->in_suspend) {
		pr_err("Already in suspend state\n");
		return 0;
	}
//[PLATFORM]-Add-END by TCTNB.ZXZ
      ...
      if(hw_tpd_double_click_switch == 1)
		{
			...
			while(msg_gesturemoderetval == 0){
			 ...
			 msg2138_data->in_suspend = true;//[PLATFORM]-Add-BEGIN by TCTNB.ZXZ, PR-967705  2015/04/07,add for TP suspend flag
			return 1;
		}
#endif
...
pwr_off_fail:
 ...
 int msg2138_ts_resume(struct device *dev)
{
    int err;
...
//[PLATFORM]-Add-BEGIN by TCTNB.ZXZ, FR-939401 2015/03/06,add for TP Psensor
	...
	if(tpd_proximity_flag2)
	{
		if(tpd_proximity_flag_one2 == 1)
		{
			tpd_proximity_flag_one2=0;
		/*[BUGFIX]-Add by TCTNB.ZXZ,2015/02/05, add for virtual Psensor can't wake up lcd when make call*/
			if (device_may_wakeup(&msg2138_data->client->dev))
...
//modify(add) by junfeng.zhou.sz for [TP driver development(1st source) for alto5] with PR721418 begin . 20140818
	...
	msg2138_data->in_suspend = false;//[PLATFORM]-Add-BEGIN by TCTNB.ZXZ, PR-967705  2015/04/07,add for TP suspend flag
...
#ifdef CONFIG_TCT_8X16_ALTO4
//919=902+17
#define GPIO_BOARD_ID4			919

int board_id_detect2(void)
{
	int rc = 0;

	rc = gpio_request(GPIO_BOARD_ID4, "board_id0");
	if (rc) {
		pr_err("==board_id_detect2=======request gpio17 failed, rc=%d\n", rc);
		goto err_gpio17;
	}

	rc = gpio_direction_input(GPIO_BOARD_ID4);

	rc=gpio_get_value(GPIO_BOARD_ID4);

		pr_err("==board_id_detect2=========gpio_get_value17==%d========\n", rc);
		gpio_free(GPIO_BOARD_ID4);
...
/*[BUGFIX]-Begin Add by TCTNB.ZXZ, PR983680,2015/04/21, add for virtual Psensor can't wake up lcd when make call*/
    wake_lock_init(&msg2138_data->msg_nosuspend_wl,WAKE_LOCK_SUSPEND, "ft_int_wakelock");
/*[BUGFIX]-End Add by TCTNB.ZXZ*/
...
/*[PLATFORM]-Add-BEGIN by TCTNB.ZXZ, PR929466 2015/02/13 ,FAE suggest add a reset to go out DebugMode(check IC ID in debug mode) in case some unknow problem  */
	tp_reset();
/*[PLATFORM]-Add-END by TCTNB.ZXZ */
...
//modify(add) by junfeng.zhou.sz for [TP driver development(1st source) for alto5] with PR721418 end .
	...
	if ( version_cat()== 1 ){
		...
		firmware_update();
		/*[PLATFORM]-Add-BEGIN by TCTNB.ZXZ, PR929466 2015/02/13 ,FAE suggest add a reset to go out DebugMode (firmware_update in debug mode) in case some unknow problem  */
		tp_reset();
		/*[PLATFORM]-Add-END by TCTNB.ZXZ*/
		version_cat();
	}
...
//[PLATFORM]-Add-BEGIN by TCTNB.ZXZ, FR-939401 2015/03/06,add for TP Psensor
	...
	if(0==board_id)
		{
			err = virtual_psensor_input_register2(client);
			if(err < 0)
			{
				pr_err("***%s: failed\n", __func__);
				return err;
...
/*[BUGFIX]-Begin Add by TCTNB.ZXZ, PR983680,2015/04/21, add for virtual Psensor can't wake up lcd when make call*/
    wake_lock_destroy(&msg2138_data->msg_nosuspend_wl);
/*[BUGFIX]-End Add by TCTNB.ZXZ*/
//modify(add) by junfeng.zhou.sz for [TP driver development(1st source) for alto5] with PR721418 begin . 20140722
