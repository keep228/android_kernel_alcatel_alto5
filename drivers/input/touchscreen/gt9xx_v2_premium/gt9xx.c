...
//modify double tap wake up the screen failure by ke.li for PR-911394  [2015.1.22]   Start
		...
		pr_err("gesture invalid  command! \n");
		return -1;
	}
...
//[FEATURE]-Modify by TCTNB.ZXZ 2015/4/28 add config version
    return sprintf ( buf, "fwversion:0x%x fwhostversion:0x%x cfgversion:0x%x \n fwmoduleid:%s  \n",ts->ic_vid,fw_info,grp_cfg_version,fw_pid);//%x-->%d,for hardware demand
}
...
/*[PLATFORM]-Mod-begin by TCTNB.ZXZ, FR988175, 2015/04/30,free reset and interupt GPIO when no GT TP*/
	goto free_gpio_int_rst;
...
/*[PLATFORM]-Mod-begin by TCTNB.ZXZ, FR988175, 2015/04/30,free reset and interupt GPIO when no GT TP*/
free_gpio_int_rst:
	if (gpio_is_valid(gt_reset_gpio))
	GTP_GPIO_FREE(gt_reset_gpio);
	if (gpio_is_valid(gt_irq_gpio))
	GTP_GPIO_FREE(gt_irq_gpio);
/*[PLATFORM]-Mod-end by TCTNB.ZXZ*/
    goodix_power_off(ts);
