...
//#define USE_TRACEBILITY		/* [BUFFIX]-Del- by TCTNB.XQJ,PR-877925, 2014/12/19,Remove tracebility ,not necessary*/
#define ALS_POLL
#ifdef ALS_POLL
...
/* [BUFFIX]-Mod- by TCTNB.XQJ,PR-877925, 2014/12/19, p sensor debug*/
#define TAOS_THD_H_OFFSET 320	// mod by ning.wei for opimize p-sensor pr743248 2014-08-21
#define TAOS_THD_L_OFFSET 120	// mod by ning.wei for opimize p-sensor pr743248 2014-08-21
...
/* [BUFFIX]-Mod- by TCTNB.XQJ,PR-890397, 2014/12/30,disable light senosr auto adjust,its done in framework */
...
static int taos_irq_clr_mask(struct tsl2772_chip *chip ) /* [BUFFIX]-Add- by TCTNB.XQJ,PR-925962, 2015/02/28,use other ways to  clear irq again */
{
      u8 bak_enable;
       u8 rc;
       rc = taos_i2c_read(chip, TSL277X_ENABLE, &bak_enable);
	if((bak_enable & TSL277X_EN_PRX_IRQ) ==0)
       {
           TSLX( "%s  bak_enable=0x%x,directy return  \n",__func__,bak_enable);
           return 0;
	}
       rc = taos_i2c_write(chip, TSL277X_ENABLE, 0x07);
       udelay(500);
       rc = taos_i2c_write(chip, TSL277X_ENABLE,bak_enable );
       if(rc)
       {
              printk(KERN_ERR "%s fail, rc=%d", __func__, rc);
              return rc;
         }
       TSLX( "%s done bak_enable=0x%x\n",__func__,bak_enable);
        return rc;
}
...
/* [BUFFIX]-Add-Begin by TCTNB.XQJ,PR-876372, 2015/1/13,adjust light sensor value*/
#if defined CONFIG_TCT_8X16_ALTO5
		lux =lux*100/460;
#elif defined CONFIG_TCT_8X16_ALTO5_PREMIUM
              lux =lux*10/52;
#endif
/* [BUFFIX]-End- by TCTNB.XQJ*/
...
exit_clr: 
	...
	taos_irq_clr_mask(chip);/* [BUFFIX]-Add- by TCTNB.XQJ,PR-925962, 2015/02/28,use other ways to  clear irq again */
	return ret;
}
...
#if 0 ///* [BUFFIX]-Del- by TCTNB.XQJ,PR-952998, 2015/03/24,this interface will not be used*/
