...
/*NB.THW 2014/11/17 add for FR.789207*/
#ifdef CONFIG_SND_QUAT_MI2S
	struct pinctrl_state *ext2_lines_sus;
	struct pinctrl_state *ext2_lines_act;
#endif
/*THW 2014/11/17 end*/
};
...
/* [PLATFORM]-Add-BEGIN by TCTNB.HJ, 2015/01/31 */
...
#define S(X, Y) ((WCD_MBHC_CAL_PLUG_TYPE_PTR(msm8x16_wcd_cal)->X) = (Y))
	S(v_hs_max, 1500);
#endif
/* [PLATFORM]-Add-END by TCTNB.HJ, 2015/01/31 */
...
#undef S
	 ...
	 */
/*NB.THW modify for PR.838829  2014/11/20*/
#ifdef CONFIG_TCT_8X16_COMMON
	btn_low[0] = 112;
	btn_high[0] = 112;
#if 0
...
int get_cdc_gpio_lines(struct pinctrl *pinctrl, int ext_pa)
{
	...
	if(ext_pa & QUAT_MI2S_ID) {
		pr_debug("%s,QUAT_MI2S_ID   \n", __func__);
		pinctrl_info.ext2_lines_sus = pinctrl_lookup_state(pinctrl,
			"cdc_lines_quat_ext_sus");
		if (IS_ERR(pinctrl_info.ext2_lines_sus)) {
			pr_err("%s: Unable to get pinctrl disable state handle\n",
								__func__);
			return -EINVAL;
		}
		pinctrl_info.ext2_lines_act = pinctrl_lookup_state(pinctrl,
			"cdc_lines_quat_ext_act");
		if (IS_ERR(pinctrl_info.ext2_lines_act)) {
			pr_err("%s: Unable to get pinctrl disable state handle\n",
								__func__);
			return -EINVAL;
		}
...
/* [PLATFORM]-Mod-BEGIN by TCTNB.HJ, 2015/01/28*/
#if defined (CONFIG_TCT_8X16_ALTO4)
int alto4_judge_boardID(int value0, int value1, int value2)
{
    int value = 0;
    ...
    if((value>=0x00) && (value<=0x04)){
        pr_debug("hujin no dis PA boost. %s-%s-%s-%s-%s\n", (value==0x00)?"mockup":"null", \
            (value==0x01)?"proto":"null", (value==0x02)?"pio01":"null", \
            (value==0x03)?"pio02":"null", (value==0x04)?"pio03":"null");
        alto4_boost_enable = 0;//1;
    }else{
        pr_debug("hujin dis PA boost. %s-%s-%s\n", (value==0x05)?"lot0":"null", \
            (value==0x06)?"lot1":"null", (value==0x07)?"lot2":"null");
        alto4_boost_enable = 0;
    }
    return value;
}
int alto4_get_boardID(struct platform_device *pdev)
{
    int value_0, value_1, value_2;
    int board_id_alto4_bit0, board_id_alto4_bit1, board_id_alto4_bit2;
#if 0
	struct pinctrl *pinctrl;
    struct pinctrl_state *board_id_alto4_det_act;
    struct pinctrl_state *board_id_alto4_det_sus;
#endif

	board_id_alto4_bit0 = of_get_named_gpio(pdev->dev.of_node,
					"board_id,alto4_bit0", 0);
	board_id_alto4_bit1 = of_get_named_gpio(pdev->dev.of_node,
					"board_id,alto4_bit1", 0);
	board_id_alto4_bit2 = of_get_named_gpio(pdev->dev.of_node,
					"board_id,alto4_bit2", 0);
	if ((board_id_alto4_bit0 < 0) || (board_id_alto4_bit1 < 0) || (board_id_alto4_bit2 < 0)){
		dev_err(&pdev->dev, "node board_id,alto4_bits NULL %d %d %d\n", board_id_alto4_bit0,
            board_id_alto4_bit1, board_id_alto4_bit2);
	}else{
		if ((!gpio_is_valid(board_id_alto4_bit0))||(!gpio_is_valid(board_id_alto4_bit1))
            ||(!gpio_is_valid(board_id_alto4_bit2))) {
			pr_err("hujin %s: Invalid gpio: %d", __func__, __LINE__);
			return -EINVAL;
		}
#if 0
		pinctrl = devm_pinctrl_get(&pdev->dev);
		...
		/* get pinctrl handle for cross det pin*/
		board_id_alto4_det_sus = pinctrl_lookup_state(pinctrl,
							"board_id_alto4_det_sus");
		if (IS_ERR(board_id_alto4_det_sus)) {
			pr_err("%s: Unable to get pinctrl disable handle\n",
								  __func__);
			return -EINVAL;
		}
		board_id_alto4_det_act = pinctrl_lookup_state(pinctrl,
							"board_id_alto4_det_act");
		if (IS_ERR(board_id_alto4_det_act)) {
			pr_err("%s: Unable to get pinctrl active handle\n",
								 __func__);
			return -EINVAL;
		}
	...
	value_0 = gpio_get_value_cansleep(board_id_alto4_bit0);
	value_1 = gpio_get_value_cansleep(board_id_alto4_bit1);
	value_2 = gpio_get_value_cansleep(board_id_alto4_bit2);
#if 0
	ret = pinctrl_select_state(pinctrl_info.pinctrl,
				pinctrl_info.cross_conn_det_act);
	if (ret < 0) {
	...
	alto4_judge_boardID(value_0, value_1, value_2);
